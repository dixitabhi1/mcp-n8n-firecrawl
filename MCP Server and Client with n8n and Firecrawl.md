...
3. Import `SCRAPINGAGENT.json` for the client workflow
4. Import `MCPserverFirecrwal.json` for the server workflow

### 3. Configure Credentials

#### Google Gemini API
1. Go to **Credentials** in your n8n instance
2. Create a new credential for "Google Gemini(PaLM) Api"
3. Enter your Google Gemini API key

#### Firecrawl API
1. Update the Authorization header in the scrape tool
2. Replace `fc-1471844158c0428ea5980bea02469c8c` with your Firecrawl API key

#### HTTP Header Authentication
1. Create credentials for "HTTP Header Auth"
2. Configure the header authentication for MCP server access

## ⚙️ Configuration

### Server Setup

1. **Activate the MCP Server Workflow**
   - Open the `MCP server Firecrwal` workflow
   - Click **Activate** to start the server
   - Note the webhook URL generated

2. **Configure ngrok (if using local n8n)**
   ```bash
   ngrok http 5678
   ```
...
2. **Activate the Client Workflow**
...
1. Create new tool nodes in the server workflow
2. Connect them to the MCP Server Trigger
3. Update the client to recognize new tool capabilities
...
- [Firecrawl](https://firecrawl.dev/) for reliable web scraping capabilities
- [Google Gemini](https://ai.google.dev/) for advanced language model integration
- [Model Context Protocol](https://modelcontextprotocol.io/) for standardized AI tool communication
...
- Documentation: [Project Wiki](https://github.com/yourusername/mcp-n8n-firecrawl/wiki)
